 ...
 *
 * Ordered most-serious first so the report can sort by it.
 */
const RISK = {
  CRITICAL: 'Critical',
  HIGH: 'High',
...
/** Sort order (lower = more serious) for ranking findings by risk. */
const RISK_ORDER = {
  [RISK.CRITICAL]: 0,
  [RISK.HIGH]: 1,
  [RISK.MEDIUM]: 2,
...
const config = {
  ...
  popiaLogging: {
    ...
    risk: RISK.MEDIUM,
    name: 'POPIA: personal info in logs',
  ...
  popiaFieldHandling: {
    ...
    risk: RISK.MEDIUM,
    name: 'POPIA: personal info handling',
...
function generateGitHubSummary() {
  ...
  categories.forEach(({ name, key, severity, risk }) => {
    ...
    if (!result.pass && result.issues.length > 0) {
      ...
      markdown += '| Risk level | File | Issue | Remediation |\n';
      markdown += '|------------|------|-------|-------------|\n';
      ...
      result.issues.forEach((issue) => {
        ...
        markdown += `| ${riskBadge(risk)} | ${fileLink} | ${escapedMessage} | ${remediationPreview} |\n`;
      });
