...
This policy is **enforced inline during BUILD** by `design-style-agent` (on-demand) and **monitored continuously during BUILD** by the developer and code-reviewer agents.
...
6. **No styling in `documentation/` artifacts dictates production directly.** Prototype-imported `tokens.css` files are *inputs* to `intake-agent` (and later `design-style-agent` if invoked), not the final production tokens. They are reviewed, merged into the Shadcn convention, and the user signs off at Gate 1.

7. **Brief-level styling stays high-level.** During INTAKE, `project-brief.md` §11 captures palette (raw hex) and font family only. Component-specific styling (button radii, card shadows, etc.) emerges on demand during BUILD — not at INTAKE.

---

## INTAKE Stage Behaviour

The `intake-agent` populates `project-brief.md` §11 with raw hex values. Two patterns:
...
The user edits these freely at Gate 1 (free-text or direct file edit). Whatever is approved becomes the input to `design-style-agent` when it's invoked during BUILD.
...
1. Reads `project-brief.md` §11 and any user-provided `documentation/tokens.css` or `documentation/genesis.md` styling section
...
## BUILD Stage Enforcement — code-reviewer
