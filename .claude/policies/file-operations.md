...
| **Dev tools** | `npx vitest`, `npm --prefix web run test`, `node_modules/.bin/tsc` | Pass Node memory/V8 flags via `NODE_OPTIONS='--max-old-space-size=256' npx vitest run` — NOT a direct `node ./node_modules/vitest/vitest.mjs` call (not auto-approved) |
