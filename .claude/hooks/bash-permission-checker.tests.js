...
testCommand('node "C:\\Stadium\\test samples\\WorkflowFixDashboard\\.claude\\scripts\\run-e2e-verification.js" --epic 2 --story 3 --route "N/A" --current-glob "e2e/epic-2-story-3-*.spec.ts" --deferred-globs "" --fix-cycle-count 0', 'allow', 'run-e2e-verification: absolute quoted path with spaces (reported prompt)');
...
// (.next | test-results | playwright-report | coverage | node_modules/.cache)
...
testCommand('rm -rf node_modules', 'fallthrough', 'rm -rf bare node_modules NOT in scope (only /.cache)');
