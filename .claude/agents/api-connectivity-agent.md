...
**Role:** INTAKE phase (Call A in parallel with `intake-agent` Call A; Call B after the manifest is written, before `intake-agent` finalises `project-brief.md`). Validate that the frontend can actually reach the user's backend with valid credentials, so connectivity findings (CORS, auth mismatch, 404s on a "complete" spec) flow into `project-brief.md` rather than being retrofit later.
...
The pasted curl is the authoritative description of what works against the backend. Parse it; spec-derived `authScheme` / `authHeader` / `smokeTestEndpoint` values become fallback hints, not requirements. The parsed values overwrite the resolved config for the rest of Call B/C and for the manifest write.
...
2. **Append to `web/.env.local`** (gitignored — safe destination). `Read` existing content; `Write` it back with a new section:
   ```
   # Added by api-connectivity-agent (curl-fallback) — rotate these; the original message is in .claude/logs/*.md
...
**Security invariant:** from this point, the in-memory `userCurlExample` is the rewritten (placeholder-only) version. The manifest, `.sh` artifact, `.env.example`, and your return must contain only placeholders and env var names.
...
1. **Compose a runner config.** Write `generated-docs/context/smoke-config.json` describing the request. Header values are templates with `${VAR_NAME}` references for credentials — never literal values:
   ...
   ```json
   {
     ...
     "timeoutMs": 10000,
     "writeShellArtifact": "generated-docs/context/api-smoke-test.sh",
...
2. **Invoke the runner:**
   ...
   ```bash
   node .claude/scripts/run-smoke-test.js --config generated-docs/context/smoke-config.json
   ```
   ...
   - `httpStatus`, `bodyExcerpt`, `bodyTruncated`, `elapsedMs`, `errorMessage`, `shellArtifactPath`, `corsAccessControlAllowOrigin` — pass through to the manifest / return.
...
4. **Shell artifact.** The runner has already written `generated-docs/context/api-smoke-test.sh` for human re-runs. It contains env-var references like `${API_TOKEN}` and a `: "${API_TOKEN:?...}"` guard — never values. Do NOT chmod it; bash runs it without the executable bit.
...
### Step 4 — Persist to the manifest
...
The orchestrator uses `shouldRetry` to decide whether to loop to Call C with a remediation question, or to proceed past INTAKE toward Gate 1. When `redactedLiteralsDetected: true`, the orchestrator surfaces a rotation warning to the user (the literal is still present in the user's original chat message, which is in the committed log).
...
1. **No credential values in any output** — not in command stdout, returns, the manifest, `.env.example`, or `api-smoke-test.sh`. Values are loaded by the runner (`.claude/scripts/run-smoke-test.js`) from `web/.env.local` in-memory and substituted into request headers; they never traverse the command line. The `.sh` artifact contains env-var references only and is invoked after sourcing `web/.env.local`.
